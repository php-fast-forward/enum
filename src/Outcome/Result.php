...
namespace FastForward\Enum\Outcome;
...
enum Result: string implements DescribedEnumInterface, LabeledEnumInterface
{
    use Comparable;
    use HasLabel;
    use HasNameLookup;
    ...
    {
        return match ($this) {
            self::Success => 'Operation completed successfully without notable issues.',
            self::Partial => 'Operation completed only in part and may require follow-up.',
            self::Failure => 'Operation did not complete successfully.',
        };
    }
    ...
    {
        return $this->in([self::Success, self::Partial]);
    }
